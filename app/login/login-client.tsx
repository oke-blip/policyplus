...
export function LoginPageClient({
  ...
  const [fetchedSettings, setFetchedSettings] = useState<Record<string, unknown> | null>(
    null,
  );
  ...
  const { logoUrl, companyName } = useMemo(() => {
    if (!settingsSource) {
          ...
          <div className="flex flex-col items-center mb-10">
            <motion.div whileHover={{ scale: 1.05 }} className="mb-6">
              {logoUrl ? (
                ...
                <div className="w-16 h-16 bg-slate-900 dark:bg-white rounded-2xl flex items-center justify-center text-white dark:text-slate-900 shadow-xl">
                  <Sparkles size={32} />
                </div>
              )}
            </motion.div>
